...
5. (Optional but recommended for testing) In **Authentication -> Providers -> Email**, you can turn off "Confirm email" while developing, so new signups can log in immediately without clicking a confirmation link. Turn it back on before sharing the app publicly.
...
- There's no "forgot password" flow wired up yet; Supabase supports it, it just isn't built into this UI. Ask if you want it added.
